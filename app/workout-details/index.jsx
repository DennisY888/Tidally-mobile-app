...
export default function WorkoutDetails() {
  ...
  const handleExerciseOptions = (exercise, index) => {
    ...
    // Find the exercise index if it's not provided
    let exerciseIndex = index;
    if (exerciseIndex === undefined || exerciseIndex === null) {
      exerciseIndex = workoutExercises.findIndex(ex => 
    ...
    // Check if we have a valid index now
    if (exerciseIndex === -1 || exerciseIndex === undefined) {
      exerciseIndex = workoutExercises.findIndex(ex => ex.name === exercise.name);
    }
